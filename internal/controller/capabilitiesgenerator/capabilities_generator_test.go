...
//go:embed test_data/wms_layer_without_name_input_expected.yaml
var WMSLayerWithoutNameExpectedInput string
...
func TestGetInputForWMSLayerWithoutName(t *testing.T) {
	type args struct {
		WMS       *pdoknlv3.WMS
		ownerInfo *smoothoperatorv1.OwnerInfo
	}
	url, _ := smoothoperatormodel.ParseURL("http://localhost/datasetOwner/dataset/theme/wms/v1_0")
	pdoknlv3.SetHost("http://localhost")
	accessConstraints, _ := smoothoperatormodel.ParseURL("http://creativecommons.org/publicdomain/zero/1.0/deed.nl")
	tests := []struct {
		name             string
		...
		{
			name: "GetInputForWMSLayerWithoutStylesWithoutName",
			args: args{
				WMS: &pdoknlv3.WMS{
					...
					ObjectMeta: metav1.ObjectMeta{
						Name:      "tno-bro-geologisch-booron2861f-v2-0",
						Namespace: "services",
					},
					Spec: pdoknlv3.WMSSpec{
						...
						Service: pdoknlv3.WMSService{
							BaseService: pdoknlv3.BaseService{
								Title:             "BRO - Geologisch booronderzoek (BHR-G) - Geologie (INSPIRE geharmoniseerd) WMS",
								URL:               smoothoperatormodel.URL{URL: url},
								AccessConstraints: smoothoperatormodel.URL{URL: accessConstraints},
							},
							DataEPSG: "EPSG:4258",
							StylingAssets: &pdoknlv3.StylingAssets{
								BlobKeys: []string{
									"resources/fonts/bzk/geologie-bro-bhrp/wingdng2.ttf",
								},
								ConfigMapRefs: []pdoknlv3.ConfigMapRef{
									{
										Name: "tno-bro-geologisch-booron2861f-v2-0-styles-2cm978b5c8",
										Keys: []string{
											"default_circle.symbol",
											"default_line.style",
											"default_point.style",
											"default_polygon.style",
											"geologicalSurvey.style",
											"geologicalSurvey.symbol",
										},
									},
								},
							},
							...
							Layer: pdoknlv3.Layer{
								Title:    ptr.To("BRO - Geologisch booronderzoek (...) WMS"),
								Abstract: ptr.To("Deze dataset bevat de gegevens over Geologische booronderzoeken..."),
								Keywords: []string{
									"BHR-G",
									"Geologie",
									"Hydrologie",
								...
								Layers: []pdoknlv3.Layer{
									{
										Name:     ptr.To("GE.Borehole"),
										Title:    ptr.To("Boorgaten"),
										Abstract: ptr.To("Een boorgat is de algemene term..."),
										Keywords: []string{"Boorgat"},
										Visible:  true,

										Styles: []pdoknlv3.Style{
											{
												Name:          "GE.Borehole",
												Title:         ptr.To("Boreholes - geologicalSurvey"),
												Visualization: ptr.To("geologicalSurvey.style"),
											},
										},
			...
			wantInput:        WMSLayerWithoutNameExpectedInput,
			isExpectingError: false,
			expectedError:    nil,
		},
		{
			name: "GetInputForWMSLayerWithStylesWithoutName",
			args: args{
				WMS: &pdoknlv3.WMS{
					TypeMeta: metav1.TypeMeta{
						APIVersion: "pdok.nl/v3",
			...
			expectedError:    errors.New("layer with styles and without name is not allowed"),
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			gotInput, err := GetInput(tt.args.WMS, tt.args.ownerInfo)

			if !tt.isExpectingError {
				if err != nil {
					t.Errorf("GetInput() error = %v, wantErr %v", err, tt.isExpectingError)
					return
				}
				wantMap := capabilitiesgenerator.Config{}
				gotMap := capabilitiesgenerator.Config{}
